...
// Fix: Using React.FC to handle 'key' and other React props correctly when used in lists
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-cyan selection:text-black">
      <Navbar />
      ...
      <section className="relative pt-48 pb-32 overflow-hidden px-6">
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <h1 className="text-5xl md:text-8xl font-heading font-bold leading-[1.1] mb-8 max-w-5xl">
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            <button className="bg-cyan text-black px-10 py-5 rounded-sm font-bold text-lg hover:scale-[1.02] active:scale-95 transition-all glow-cyan flex items-center justify-center gap-3">
        ...
        {/* Abstract Background */}
        <div className="absolute top-0 right-0 w-1/2 h-full bg-[radial-gradient(circle_at_70%_30%,rgba(0,255,255,0.1),transparent_70%)] pointer-events-none opacity-50"></div>
        <div className="absolute inset-0 bg-grid opacity-20 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-white/10 to-transparent"></div>
      </section>

      {/* Problema Section */}
      <section id="problema" className="py-32 bg-[#050505] px-6 border-b border-white/5">
          ...
          <div className="grid lg:grid-cols-2 gap-24 items-center">
            <div>
              ...
              <p className="text-gray-400 text-lg mb-10 leading-relaxed">
                Gestores estão crescendo, mas a infraestrutura operacional continua fragmentada e dependente de intervenção humana constante. Isso gera um teto de crescimento invisível.
              </p>
              <div className="grid gap-6">
                {[
                  "Processamento manual com alto risco de erro",
                  "Falta de rastreabilidade entre documento e decisão",
                  "Reconciliação frágil with registradoras (B3/BCB)",
                  ...
                  <div key={i} className="flex items-center gap-4 text-gray-300 group">
                    <div className="p-1 rounded-full bg-red-500/10 border border-red-500/20 group-hover:bg-red-500/20 transition-colors">
            ...
            <div className="grid grid-cols-2 gap-6 relative">
              <div className="bg-[#0A0A0A] border border-white/5 p-10 rounded-none flex flex-col items-center justify-center text-center">
                <span className="text-6xl font-heading font-bold text-cyan mb-2">60%</span>
                <span className="text-gray-500 text-sm uppercase tracking-widest font-bold">Menos tempo operacional</span>
              </div>
              <div className="bg-[#0A0A0A] border border-white/5 p-10 rounded-none flex flex-col items-center justify-center text-center lg:translate-y-12">
                <span className="text-6xl font-heading font-bold text-cyan mb-2">30%</span>
                <span className="text-gray-500 text-sm uppercase tracking-widest font-bold">Menos custo de execução</span>
              </div>
              {/* Decorative square */}
              <div className="absolute -top-4 -left-4 w-8 h-8 border-t-2 border-l-2 border-cyan/30"></div>
              <div className="absolute -bottom-4 -right-4 w-8 h-8 border-b-2 border-r-2 border-cyan/30 translate-y-12 lg:translate-y-24"></div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Agents Grid Section */}
      <section id="agentes" className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <p className="text-gray-500 font-mono text-sm max-w-sm">
              Arquitetura agêntica brikz v2.5 // Auditável // Escalável // FIDC Compliant
            </p>
          </div>
      ...
      <section className="py-32 px-6">
        <div className="max-w-7xl mx-auto bg-[#0A0A0A] border border-white/10 p-12 md:p-24 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2 className="text-4xl md:text-5xl font-heading font-bold mb-20">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-16">
              {[
                ...
                <div key={i} className="flex flex-col gap-6">
                  <div className="w-8 h-1 bg-cyan/30"></div>
      ...
      {/* Prova / Resultados */}
      <section id="prova" className="py-32 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-3 gap-16">
            {[
              ...
              <div key={i} className="group">
                <div className="mb-8 p-3 w-fit bg-white/5 border border-white/10 group-hover:border-cyan/50 transition-colors">
                  {caseStudy.icon}
                </div>
                <p className="text-xs uppercase tracking-[0.25em] text-cyan/70 font-bold mb-4">{caseStudy.label}</p>
                <h3 className="text-2xl font-heading font-bold text-white mb-6 group-hover:text-cyan transition-colors leading-snug">{caseStudy.title}</h3>
                <p className="text-gray-500 font-medium flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-cyan/50"></div>
                  Parceiro: {caseStudy.client}
                </p>
              </div>
            ))}
          </div>
      ...
      {/* CTA Final */}
      <section className="py-32 px-6 bg-cyan text-black text-center relative overflow-hidden group">
        <div className="max-w-4xl mx-auto relative z-10">
          <h2 className="text-5xl md:text-8xl font-heading font-bold mb-10 tracking-tight leading-[0.9]">
            Pronto para escalar <br />sem inflar operação?
          </h2>
          <p className="text-black/70 text-xl md:text-2xl mb-14 font-medium max-w-2xl mx-auto">
            Transforme elegibilidade, conciliação e monitoramento em uma vantagem competitiva injusta para seu FIDC.
          </p>
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <button className="bg-black text-white px-12 py-6 rounded-sm font-bold text-xl hover:scale-105 active:scale-95 transition-all shadow-2xl flex items-center justify-center gap-3">
              Agendar demonstração <ArrowRight />
            </button>
          </div>
          <p className="mt-16 text-black/40 font-bold uppercase tracking-widest text-sm">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] opacity-5 pointer-events-none">
          <div className="w-full h-full bg-grid-black opacity-10"></div>
          <div className="absolute top-0 left-0 w-full h-full border-[100px] border-black rounded-full scale-150 animate-pulse"></div>
        </div>
      </section>

      {/* Footer */}
      <footer className="py-24 px-6 border-t border-white/10 bg-[#020202]">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-16">
          <div className="lg:col-span-1">
            <Logo />
            <p className="mt-8 text-gray-500 text-sm leading-relaxed max-w-xs">
              Infraestrutura de dados agêntica para o mercado de capitais. Precisão, escala e transparência total para gestores de FIDC.
            </p>
          </div>
          <div>
            <h4 className="text-white font-bold text-xs mb-8 uppercase tracking-[0.2em]">Produto</h4>
            <ul className="space-y-5 text-gray-500 text-sm font-medium">
              <li><a href="#" className="hover:text-cyan transition-colors">Esteira de Crédito</a></li>
              <li><a href="#" className="hover:text-cyan transition-colors">Reconciliação</a></li>
              <li><a href="#" className="hover:text-cyan transition-colors">Monitoramento</a></li>
              <li><a href="#" className="hover:text-cyan transition-colors">Auditabilidade</a></li>
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold text-xs mb-8 uppercase tracking-[0.2em]">Compliance</h4>
            <ul className="space-y-5 text-gray-500 text-sm font-medium">
              <li><a href="#" className="hover:text-cyan transition-colors">Resolução CVM 175</a></li>
              <li><a href="#" className="hover:text-cyan transition-colors">LGPD Financial</a></li>
              <li><a href="#" className="hover:text-cyan transition-colors">Segurança Bancária</a></li>
              <li><a href="#" className="hover:text-cyan transition-colors">Logs Imutáveis</a></li>
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold text-xs mb-8 uppercase tracking-[0.2em]">Contato</h4>
            <p className="text-gray-500 text-sm mb-6 font-medium">contato@brikz.com</p>
            <div className="flex gap-4">
              {[BarChart3, TrendingUp, ShieldCheck].map((Icon, i) => (
                <div key={i} className="w-10 h-10 bg-white/5 border border-white/10 flex items-center justify-center hover:border-cyan cursor-pointer transition-all hover:bg-cyan/5">
        ...
        <div className="max-w-7xl mx-auto mt-24 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
          <p className="text-gray-600 text-xs font-mono">© 2025 BRIKZ AGENTIC SOLUTIONS. ALL RIGHTS RESERVED.</p>
